...
class HGRecommendationPage extends Component {
  ...
  loadRecommendations = () => {
    ...
    HGRecommendationVM.fetchRecommendations()
      .then((result) => {
        ...
        this.setState({ rows: HGRecommendationVM.rows(result), refreshedAt: new Date() });
      })
      .catch((error) => {
        if (this.unmounted || sequence !== this.requestSequence) return;
  ...
  renderContent = (_, row) => (
    <div className={styles.contentCell}>
      ...
      <div>
        ...
        <span>{row.authorName || "未知作者"} · {row.contentId || "-"}</span>
      </div>
    </div>
  );
  ...
  getColumns = () => [
    ...
    { title: "播放", dataIndex: "viewCount", width: 110, render: HGRecommendationVM.countText },
    { title: "点赞", dataIndex: "likeCount", width: 100, render: HGRecommendationVM.countText },
    { title: "弹幕", dataIndex: "commentCount", width: 100, render: HGRecommendationVM.countText },
    { title: "时长", dataIndex: "durationSeconds", width: 90, render: HGRecommendationVM.durationText },
    { title: "发布时间", dataIndex: "publishedAt", width: 190, render: (value) => value ? new Date(value).toLocaleString() : "-" },
    { title: "原站", dataIndex: "targetUrl", width: 100, render: (value) => value ? <a className={styles.sourceLink} href={value} target="_blank" rel="noreferrer noopener">查看</a> : "-" },
  ];
      ...
      <div>
        <div className={styles.header}>
          <div>
            <p>COLLECTED DATA</p>
            <h1>采集结果</h1>
            <span>查看最近一次成功任务产出的公开元数据快照，不下载或代理媒体文件</span>
          </div>
          <HGButtonPage loading={loading} onClick={this.loadRecommendations}>刷新快照</HGButtonPage>
        </div>
        <div className={styles.summary}>
          <div><span>快照记录</span><strong>{rows.length}</strong></div>
          <div><span>数据来源</span><strong>Bilibili Public API</strong></div>
          <div><span>页面刷新</span><strong>{refreshedAt ? refreshedAt.toLocaleTimeString() : "等待加载"}</strong></div>
        </div>
        <HGCardPage title="最新成功快照" extra={<span className={styles.note}>失败任务不会覆盖上一份可用数据</span>}>
          <HGTablePage rowKey={(row) => `${row.platform}:${row.contentId}`} columns={this.getColumns()} dataSource={rows} loading={loading} pagination={false} scroll={{ y: 480 }} />
          {!loading && rows.length === 0 ? <div className={styles.empty}>尚无成功采集快照，请先在任务管理中执行推荐采集。</div> : null}
        </HGCardPage>
      </div>
    );
  }
