...
class HGBiliAuthorSpacePage extends React.Component {
  ...
  getUserId = () =>
    decodeURIComponent(String(this.props.params?.userId || "").trim());
      ...
      <>
        ...
        <section className={styles.navBar}>
          ...
          <div className={styles.stats}>
            <span>
              <strong>
                {HGBiliAuthorSpacePageVM.formatCount(stats.following)}
              </strong>
              关注数
            </span>
            <span>
              <strong>
                {HGBiliAuthorSpacePageVM.formatCount(stats.followers)}
              </strong>
              粉丝数
            </span>
          </div>
    ...
    return (
      <section className={styles.contentCard}>
        <div className={styles.sectionTitle}>
          <div>
            <span className={styles.accent} /> <h2>TA的视频</h2>
          </div>
          <span>
            共 {HGBiliAuthorSpacePageVM.formatCount(stats.videos)} 个投稿
          </span>
        </div>
        {videos.length ? (
              ...
              >
                <span className={styles.coverWrap}>
                  {video.cover ? (
                    ...
                    <span className={styles.coverFallback}>bilibili</span>
                  )}
                ...
                <span className={styles.videoMeta}>
                  点赞 {HGBiliAuthorSpacePageVM.formatCount(video.likeCount)} ·{" "}
                  {HGBiliAuthorSpacePageVM.formatDate(video.publishTime)}
                </span>
              </button>
            ))}
          </div>
  ...
  render() {
    ...
    if (loading)
      return <div className={styles.statePage}>正在进入个人空间...</div>;
    if (!profile)
      return <div className={styles.statePage}>{error || "用户不存在"}</div>;
    return (
        ...
        <div className={styles.shell}>
          ...
          <div className={styles.bodyGrid}>
            ...
            <aside className={styles.sideCard}>
              ...
              <p>
                <span>加入时间</span>
                {HGBiliAuthorSpacePageVM.formatDate(profile.createdAt) ||
                  "未知"}
              </p>
              <p>
                <span>性别</span>
