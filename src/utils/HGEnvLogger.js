 ...
 */
function buildPrefix(env, level) {
  const envIcon = ENV_ICON_MAP[env] ?? ENV_ICON_MAP.product;
  const levelIcon = LEVEL_ICON_MAP[level] ?? LEVEL_ICON_MAP.info;
...
export default class HGEnvLogger {
  ...
  static getCurrentEnv() {
    return normalizeEnv(import.meta.env?.VITE_APP_ENV);
  }
  ...
  static log(level, ...args) {
    ...
    const prefix = buildPrefix(env, normalizedLevel);
